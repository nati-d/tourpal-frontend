...
const PlanResultDialog = ({open, handleCloseModal, plans}: PlanDialogProps) => {
	return (
		...
		>
			<DialogContent className='max-w-full w-full h-[90vh] bg-white dark:bg-gray-800 overflow-y-auto rounded-lg'>
				<DialogHeader>
						...
						{plans?.DaysPlan && (
							<Card className='flex flex-col gap-4'>
								<CardHeader className='text-xl font-bold'>
									<CardTitle className="flex items-center gap-2"><Calendar size={18}/>Daily Itinerary</CardTitle>
								</CardHeader>
								<CardContent className='space-y-4'>
									{plans.DaysPlan.map((day: any, index: number) => {
														...
														<li key={activityIndex}>
															<Card className='w-full shadow-md border rounded-lg bg-gray-50 dark:bg-gray-800'>
																<CardHeader className='flex flex-row items-center gap-2'>
																	{activity.Time === "Morning" && <Sun className='text-yellow-500 w-5 h-5' />}
																	{activity.Time === "Afternoon" && <CloudSun className='text-orange-500 w-5 h-5' />}
																	{activity.Time === "Evening" && <Moon className='text-purple-500 w-5 h-5' />}
																	<CardTitle className='text-xl font-semibold text-gray-800'>{activity.Time}</CardTitle>
																</CardHeader>
																<CardContent className='flex flex-col gap-2 text-[18px]'>
																	<span className='text-lg text-gray-700 dark:text-gray-300 font-semibold'>
																		{activity.Place}
																	</span>
																	<span className='text-base text-gray-600 dark:text-gray-400'>{activity.Description}</span>
																</CardContent>
															</Card>
														</li>
													))}
